...
export class AuditorDiagramsListService {
  ...
  private async getDiagrams(filter: DiagramFilter): Promise<void> {
    try {
      const diagrams = await this.dataProvider.AuditorDiagram.getMany(filter);
  ...
  public async aproveAll(): Promise<void> {
    try {
      let filter = new DiagramFilter();
      filter.state = DiagramState.Rechazado;
      let diagramas = await this.dataProvider.AuditorDiagram.getMany(filter);
      for (let diagram of diagramas) {
        diagram.state = DiagramState.Aprobado;
        await this.dataProvider.AuditorDiagram.update(diagram.id_diag, diagram);
      }
      this.refresh();
    }
    catch (error) {
  ...
  private async getTotal(): Promise<void> {
    try {
      const total = await this.dataProvider.AuditorDiagram.total();
