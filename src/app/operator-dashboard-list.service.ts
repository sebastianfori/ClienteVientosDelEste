...
export class OperatorDashboardListService {
  ...
  private async getDiagrams(filter: DiagramFilter): Promise<void> {
    try {
      const diagrams = await this.dataProvider.UserDiagram.getMany(filter);
  ...
  private async getTotal(): Promise<void> {
    try {
      const total = await this.dataProvider.UserDiagram.total();
